        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (cbxPeopleInvolved.SelectedIndex == -1)
            {
                ...
                else
                {
                    ...
                    patient.FullName = txtFullName.Text;
                    patient.YearOfBirth = long.Parse(txtYearOfBirth.Text);
                    string cmnd = r.Cells["CMND"].Value.ToString();
                    var pa = db.Patients.SingleOrDefault(x => x.CMND.Equals(cmnd));
                    var add = db.Addresses.SingleOrDefault(x => x.IDAddress == pa.IDAddress);
                    add.IDWard = int.Parse(cbxWard.SelectedValue.ToString()); 
                    add.IDCity = int.Parse(cbxCity.SelectedValue.ToString());
                    add.IDDistrict = int.Parse(cbxDistrict.SelectedValue.ToString());
                    add.Street = txtAddress.Text;
